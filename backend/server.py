...
async def create_company(company: CompanyCreate, current_user: User = Depends(get_current_user)):
    ...
    # Send SMS
    message = f"Welcome to IT Signature ERP! Your company '{company.name}' has been created. Login with mobile {company.admin_mobile}. URL: https://payroll-genius-6.preview.emergentagent.com"
...
async def get_detailed_payroll(month: str, current_user: User = Depends(get_current_user)):
    ...
    for employee in employees:
        ...
        if employee.get("fixed_salary", False):
            # Fixed salary: always get full basic + allowances regardless of attendance
            earnings = basic_salary + allowances
            gross_salary = earnings + total_extra_payment  # Allowances already in earnings
        else:
